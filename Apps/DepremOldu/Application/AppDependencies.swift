...
enum UITestingEarthquakeFixtures {
    static func feed() -> [Earthquake] {
        let now = Date()
        let samples: [(String, String, Double, Double, TimeInterval)] = [
        ...
        return samples.enumerated().map { index, sample in
            Earthquake(
                ...
                occurredAt: now.addingTimeInterval(-sample.4),
                coordinate: GeoCoordinate(
                    latitude: 36.0 + Double(index) * 0.9,
                    longitude: 26.0 + Double(index) * 1.2
                )
            )
        }
    }
