...
- `AppRootView` sabit bir üst başlık çizer (solda marka, sağda Hakkında); başlık TabView dışında olduğu için sekme geçişlerinde animasyon veya yeniden kurulum olmaz. Yenileme yalnız pull-to-refresh iledir; açılışta veya stale cache yeniden doğrulanırken liste içeriğinin üstünde küçük, sistem varsayılan renkli bir yükleme göstergesi yumuşak animasyonla açılıp kapanır. Uygulama bilinçli olarak yalnız açık moddur.
...
- Sunum durumları ayrıdır: `loading` (içerik yok), `content` (fresh/stale + refresh state), `failure` (içerik yok). Refresh sırasında içerik korunur; açılış veya stale yeniden doğrulamada listenin üstünde yükleme satırı, kullanıcı pull-to-refresh'inde sistem göstergesi görünür. Sayaç `200 deprem` biçimindedir.
